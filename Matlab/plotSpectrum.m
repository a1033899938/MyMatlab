...
% 取波长范围[a b],或'all'
wavelengthRange =           'all';%'all';%[600,650];%[6.109741832228412e+02,6.189106668987079e+02];%'all';%[550 800];'all';%%%;
% 取strip的范围(不可设置1:end),小房间PI最大为1:100，ARS最大为;
spectrumRange   =           '500:700';%'52-3:52+4';
%% 05 输出image
% 纵坐标转换为angle(限image)
sw.ylabelToAngle =            0;       NA = 0.75;
% 横坐标转换为能量
sw.xlabelToEnergy =           0;
% image方向变竖直
sw.showImageVertically =      0; 
...
sw.showImageStripGraph =      0;     strip_graph_amlitude_ratio = 0.1;       strip_graph_color = '-g';        strip_graph_linewidth = 1;
% 显示wavelength-graph(strip方向累加图)
sw.showImageWavelengthGraph = 0;        wavelength_graph_amlitude_ratio = 0.2;      wavelength_graph_color = '-b';       wavelength_graph_linewidth = 1;
%
...
sw.showGraph =                1;
% graph放在一张图中
sw.showGraphInOnePlot =       0;
% 放在一张瀑布图中%必须开启sw.show_graph_in_one_plot
sw.showGraphInCascadePlot =   0;      offset_pre = 0;%常见错误：开启归一化以后使用过大的offset（>1）
